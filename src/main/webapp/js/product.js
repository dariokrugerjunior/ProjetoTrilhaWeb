...
$(document).ready(function () {
	...
	COLDIGO.produto.cadastrar = function () {
		...
		produtosTotal.map((x) => {
			if(x.modelo.toString() === produto.modelo.toString() && x.valor.toString() === produto.valor.toString() ){
				verification = true
			}
		})
		...
		if ((produto.categoria == "") || (produto.marcaId == "") || (produto.modelo == "")
			...
			COLDIGO.exibirAviso("Produto já cadastrado");
		} else {
			$.ajax({
				type: "POST",
	...
	function arrayProduct(json) {
		produtosTotal = new Array();
		var arrays = ['modelo'];
		arrays.map(function () {
			json.map(function (objeto) {
				produtosTotal.push(objeto)
			});
		});
	}
	...
	COLDIGO.produto.excluir = function (id) {
		$.ajax({
			...
			error: function (info) {
				COLDIGO.exibirAviso("Error ao excluir produto: " + info.status + " - " + info.statusText);
			}
		});
	};
